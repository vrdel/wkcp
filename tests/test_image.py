...
from wkcp.utils import extract_delete_imgpaths


class DeleteImagePathExtract(unittest.TestCase):
    def test_extracts_dash_image_label(self):
        self.assertEqual(
            extract_delete_imgpaths(["- image: image.jpg"], path=False),
            "image.jpg",
        )

    def test_extracts_dash_image(self):
        self.assertEqual(
            extract_delete_imgpaths(["- image.jpg"], path=False),
            "image.jpg",
        )
    ...
    def test_preserves_existing_markdown_image_support(self):
        self.assertEqual(
            extract_delete_imgpaths(["![alt](image.jpg)"], path=False),
            "image.jpg",
        )

    def test_prepends_current_directory_for_relative_paths(self):
        self.assertEqual(
            extract_delete_imgpaths(["image.jpg"]),
